...
contains
    ...
    use nonInputVariables, Only: num_records,     &
                                       ...
                                       DailyFlow, &   !OUTPUT (m3/sec) array of daily flow
    ...
    do
	    ...
	    if (ierror /=0 .AND. flag_read==1) then
		    ...
		    DailyFlow=DailyFlow*afield/8640000.	!m3/s :(cm/day) *(m2)* (m/100cm)* (day/86400s)
	    ...
	    do
		    ...
		    DailyFlow(absolute_day)=runoff_cm    !q is still in cm until the final return (see above)
