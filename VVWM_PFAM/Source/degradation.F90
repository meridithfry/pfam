...
use variables, ONLY: MWT,SOL,VAPR,area, wind_height
use nonInputVariables, Only: num_records, &
...
real(8) :: 	MWT_local,SOL_local,VAPR_local
real(8),dimension(num_records) :: U			!wind speed adjusted to 10m above surface

MWT_local = MWT(index)
SOL_local = SOL(index)
VAPR_local = VAPR(index)


HENRY = VAPR_local/760./(SOL_local/MWT_local)			!EXAMS p188 version 2.98
...
!******Rewrite calculate liquid resistance*********
	...
	RL = 1./RL/sqrt(32./MWT_local)			!(s/m) EXAMS (eq 2-77)
...
!	wind_10cm = U/2.	!convert to wind speed at 10 cm
	...
	RG = R*(temp_avg+273.15)/(0.1857+5.68*U)/HENRY/sqrt(18./MWT_local)
	RG = RG*3600.									    !(s/m)
!************************************************************************
