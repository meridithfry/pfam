...
contains
    ...
    !This subroutine reads the metfile, stores precip and evap in meters and also returns the 
    !average previous 30 day temperature (C) and daily wind speed (m/s)
    
     use variables, ONLY: metfilename, watershed_cn,watershed_area
     use nonInputVariables, Only: num_records, &
                                  wind,        &  !array of output wind speed (m/s)
                                  temp_avg,    &  !output: average of previous 30 day temperature (C)
                                  evap,        &  !output: daily evaporation (m)
                                  precip,      &  !output: daily precipitation (m)
                                  num_years,   &  !output: number of years in simulation
                                  runoff,      &  !output: runoff by curve number method of surrounding watershed
                                  firstyear, lastyear
    ...
    do i=1,num_records
	    ...
	    read (19,100, IOSTAT=eof) yr,precip(i), evap(i),temp,wind(i)  !wind is used directly, temp is processed later

	    100 format(5X,I2,5F10.0)  !see PRZM manual for definitions in met file p.4-2
    ...
    rain = precip*39.37   !inches
 
  S= 1000./watershed_cn-10.

   where (rain > 0.2*S)
    runoff =(rain-0.2*S)**2/(rain + 0.8*S)/39.37/86400.*watershed_area   !runoff in m3/s
   elsewhere
    runoff = 0.0
   end where
