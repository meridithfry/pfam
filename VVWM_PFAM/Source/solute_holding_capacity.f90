...
subroutine solute_holding_capacity(index)
   use variables, ONLY:  area, koc, depth_0, porosity,bulk_density,FROC1,FROC2,SUSED,DOC1,DOC2,PLMAS,BNMAS,benthic_depth
   use nonInputVariables, Only: num_records, &
                                ...
                                theta, &       !out
                                daily_depth, & !INPUT
                                v1,  &         !aqueous volume in 1, assumes SS volume is negligible
                                v2             !aqueous volume of 2
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!Default EXAMS conditions for partitioning
    KOW = koc_local/.35     !DEFAULT EXAMS CONDITION ON Kow  p.35
    KPDOC1= KOW*.074        !DEFAULT RELATION IN EXAMS (LITTORAL)
    KPDOC2 = koc_local      !DEFAULT RELATION IN EXAMS (BENTHIC) p.16 of EXAMS 2.98 (or is it Kow*.46 ?)
...
! partitioning coefficients of individual media 
    kd_sed_1 = koc_local*FROC1*.001       !Kd of sediment in littoral [m3/kg]
    kd_sed_2 = koc_local*FROC2*.001       !Kd of sediment in benthic                       
