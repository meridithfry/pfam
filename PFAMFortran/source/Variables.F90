...
!Common variables that are calculated and used internally between routines are transported as arguments.
    ...
    integer,parameter  :: max_floods =30
   ...
   ! real(8) event0_weir     !the weir ht on the reference day
    ...
    integer :: eventDay(max_floods)  !each event  day is number of days from reference day, event 1 day is always zero
    real(8) :: eventFill(max_floods)
    real(8) :: eventWeir(max_floods)
    real(8) :: eventWashout(max_floods)
    real(8) :: eventMinimum(max_floods)
    ...
    real(8):: CHL
    real(8):: DOC1 
    real(8):: DOC2 = 0.0  !not using this anymore
    real(8):: PLMAS = 0.0 !not using this anymore
    real(8):: BNMAS = 0.0
    real(8):: QT           !EXAMS Q10 values eq. 2-133
    real(8):: DFAC
    real(8):: area  !water body area (m2)
    real(8):: D_over_dx
    real(8):: leakage  !benthic leakage
    real(8):: wierleakage
